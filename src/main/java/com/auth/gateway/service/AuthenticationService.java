...
public class AuthenticationService {
    ...
    public User signUp(RegisterUserDto input)
    {
        ...
        user.setVerficationCodeExpiresAt(LocalDateTime.now().plusMinutes(15));
    ...
    public User authenticate(LoginUserDto input){
        ...
        if(!user.isEnabled()){
            throw new RuntimeException("Account not verified. Please verfiy your account");
        }
    ...
    public void verifiyUser(VerifyUserDto input){
        Optional<User> optionalUser = userRepository.findByEmail(input.getEmail());
        if(optionalUser.isPresent()){
            User user = optionalUser.get();
            if(user.getVerficationCodeExpiresAt().isBefore(LocalDateTime.now())){
                throw new RuntimeException("Verification code has expried");
            }
    ...
    public  void resentVerificationCode(String email){
        ...
        if(optionalUser.isPresent()){
            ...
            throw new RuntimeException("User is not found in the repository");
        }
    }
